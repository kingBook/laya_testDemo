...
export interface DynamicAtlasConfig {
    ...
    extendSize: number;
    /** 纹理格式 */
    textureFormat: Laya.RenderTargetFormat;
    /** 是否立即执行合并 */
    immediately: boolean;
    /** 是否自动扩展大纹理数量 */
    autoExtend: boolean;
...
export class DynamicAtlasManager {
    ...
    private _findSmallTexture(tex: Laya.Texture, info: TextureInfo) {
        let out = info.textureMap;
        let ot = tex.bitmap;
        const _find = (texture: Laya.Texture) => {
            let atlas = (<any>texture)._atlas;
            if (atlas) {
        ...
        _find(tex);
        if ((<any>tex)._dynamic != info) {
            out.set(tex.id, { texture: tex, uv: tex.uv });
            (<any>tex)._dynamic = info;
        }
    }
     ...
     * @returns 是否添加成功
     */
    public addTexture(texture: Laya.Texture, scale: number = 1.0, largeTextureIndex: number = -1): boolean {
        ...
        let texture2D = texture.bitmap as Laya.Texture2D;
        ...
        textureInfo.uv = new Laya.Vector4(
    ...
    public addTextureByUrl(url: string, scale: number = 1.0, largeTextureIndex: number = -1): boolean {
        ...
        let texture = Laya.loader.getRes(url, Laya.Loader.IMAGE) as Laya.Texture;
        if (!texture) {
     ...
     * @returns 成功添加的纹理数量
     */
    public addTextures(textures: Laya.Texture[], scale: number = 1.0, largeTextureIndex: number = -1): number {
        let successCount = 0;
    ...
    public removeTexture(textureId: number, largeTextureIndex: number = -1 , event: boolean = true): boolean {
        ...
        //还原
        let otexture2d = Laya.loader.getRes(textureInfo.url , Laya.Loader.TEXTURE2D) || textureInfo.source;
    ...
    public getTextureInfoByUrl(url: string): TextureInfo | null {
        return this.getTextureInfo(Laya.loader.getRes(url, Laya.Loader.TEXTURE2D).id) || null;
    }
