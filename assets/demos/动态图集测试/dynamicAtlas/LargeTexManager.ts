...
export class LargeTexBase {
     ...
     * @zh 纹理重复模式
     * */
    texWrap = Laya.WrapMode.Clamp;
    /** 
     * @en Texture sampling mode
     * @zh 纹理采样模式
     * */
    texMode = Laya.FilterMode.Trilinear;
    /** 
     ...
     * @zh 大纹理像素格式
     * */
    texFormat = Laya.RenderTargetFormat.R8G8B8A8;
    /** 
     * @en Background color
     * @zh 背景色
     * */
    backColor = new Laya.Vector4(90, 90, 90, 255);
    ...
    largeTexs: LargeTex[] = [];
    /** 命令缓冲区 */
    protected _cmdBuffer = new Laya.CommandBuffer2D();
     ...
     * @param exs 小纹理扩边尺寸
     */
    constructor(largeTexsize: number[], ltn: number, tsm: number = 16, exs: number = 0, texFormat: Laya.RenderTargetFormat = Laya.RenderTargetFormat.R8G8B8A8) {
        if (largeTexsize[0]) {
    ...
    calculateGpuMemory() {
        ...
        for (let i = this.largeTexs.length - 1; i > -1; i--) 
            this.gpuMemory += (this.largeTexs[i] as any)._renderTarget.gpuMemory;
    ...
    fillWithColor(largeTextureIndex: number, x: number, y: number, w: number, h: number, r: number, g: number, b: number, a: number = 255) {
        if (largeTextureIndex < this.LARGE_TEX_N && this.largeTexs[largeTextureIndex]) {
            const tex = new Laya.Texture2D(1, 1, Laya.TextureFormat.R8G8B8A8, false, false);
     ...
     * @returns [宽，高，利用率]
     */
    static calcFitTexSize(tex: Laya.Texture2D[], scale: number, tsm: number, exs: number, tp?: any) {
     ...
     * @param texs 贴图组
     */
    areAllTexturesIncluded(texs: Laya.Texture2D[]) {
        if (texs) {
     ...
     */
    hasEnoughSpace(tex: Laya.Texture2D[], scale: number, largeTextureIndex: number) {
     ...
     * @return 成功大纹理编号，失败-1
     */
    findAvailableLargeTextureCode(tex: Laya.Texture2D[], scale: number) {
        for (let i = 0; i < this.LARGE_TEX_N; i++)
     ...
     */
    addTexturesToLargeTexture(texs: Laya.Texture2D[], scale: number, largeTextureIndex: number, needRemove: boolean = false) {
     ...
     */
    addTexture(tex: Laya.Texture2D, scale: number = 1, largeTextureIndex: number = -1, url: string = null, needRemove: boolean = false) {
        if (!tex) return -1;
     ...
     * @returns 成功大纹理编号，失败-1
     */
    updateTexture(tex: Laya.Texture2D, scale: number = 1, largeTextureIndex: number = -1, url?: string, needRemove: boolean = false) {
        if (tex) {
     ...
     * @returns 纹理对象
     */
    getTextureByRef(tex: Laya.Texture2D, largeTextureIndex: number = -1) {
     ...
     * @returns 
     */
    protected _addTexture(tex: Laya.Texture2D, scale: number, largeTextureIndex: number = -1, url: string = null, needRemove: boolean = false) {
        if (!tex || tex instanceof Laya.Texture2D == false) return null;
     ...
     * @returns 
     */
    protected _updateTexture(tex: Laya.Texture2D, scale: number, largeTextureIndex: number = -1, url: string = null, needRemove: boolean = false) {
        if (!tex) return null;
...
export class LargeTexManager extends LargeTexBase {
    ...
    private _curColor: Laya.Vector3[] = []; //当前合并的颜色序号（x：块横向起始，y：块纵向起始, z: 块内序号）
    private _curColorUV: Laya.Vector2 = new Laya.Vector2(); //当前合并的颜色UV值
    ...
    addDummySideTexture(largeTextureIndex: number = -1) {
        this.addTexture(Laya.Texture2D.blackTexture, 64, largeTextureIndex, null, true);
    }
     ...
     * @returns 成功大纹理编号，失败-1
     */
    addColor(color: Laya.Vector4, largeTextureIndex: number = -1) {
     ...
     * @returns 大纹理或null
     */
    getColor(color: Laya.Vector4, uv?: Laya.Vector4, largeTextureIndex: number = -1) {
    ...
    private _findColor(r: number, g: number, b: number, a: number, largeTextureIndex: number) {
        if (largeTextureIndex >= 0 && largeTextureIndex < this.LARGE_TEX_N) {
            ...
            if (this.texFormat == Laya.RenderTargetFormat.R8G8B8) {
                for (let i = 0; i < len; i++) {
            ...
            else if (this.texFormat == Laya.RenderTargetFormat.R8G8B8A8) {
                for (let i = 0; i < len; i++) {
                    if (this._curColorMap[largeTextureIndex][i]
                        && this._curColorMap[largeTextureIndex][i].r == r
                        && this._curColorMap[largeTextureIndex][i].g == g
    ...
    private _addColor(r: number, g: number, b: number, a: number, largeTextureIndex: number) {
        ...
        if (add >= 0) {
            ...
            let cmd = this.largeTexs[add].addColor(x - this.COLOR_ITEM_SIZE * 0.5, y - this.COLOR_ITEM_SIZE * 0.5,
                this.COLOR_ITEM_SIZE, this.COLOR_ITEM_SIZE, new Laya.Vector4(r, g, b, a), this.texFormat);
            return add;
        }
        return -1;
    }
    ...
    getBase64(): Promise<string[]> {
        ...
        this.largeTexs.forEach(v => {
            promises.push(Laya.Utils.uint8ArrayToArrayBufferAsync(v));
        })
