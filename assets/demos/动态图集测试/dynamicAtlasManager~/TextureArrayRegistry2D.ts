...
export class TextureArrayRegistry2D {
    private static _idToRecord: Map<number, { array: Laya.Texture2DArray, layer: number }> = new Map();
    private static _groupKeyToArray: Map<string, { array: Laya.Texture2DArray, nextLayer: number, capacity: number }> = new Map();

    static clear(): void {
        this._idToRecord.clear();
    }

    static register(source: Laya.Texture | Laya.BaseTexture | Laya.Texture2D, array: Laya.Texture2DArray, layer: number): void {
        const id = this._getTexId(source);
        if (id == null) return;
        this._idToRecord.set(id, { array, layer });
    }

    static unregister(source: Laya.Texture | Laya.BaseTexture | Laya.Texture2D): void {
        const id = this._getTexId(source);
        if (id == null) return;
        this._idToRecord.delete(id);
    }

    static resolve(source: Laya.Texture | Laya.BaseTexture | Laya.Texture2D) {
        const id = this._getTexId(source);
     ...
     */
    static allocateLayerAsTexture(width: number, height: number, format: Laya.TextureFormat = Laya.TextureFormat.R8G8B8A8, capacity: number = 64, sRGB: boolean = false) {
        const key = `${width}x${height}_${format}_${sRGB ? 1 : 0}`;
        let group = this._groupKeyToArray.get(key);
        if (!group) {
            const array = new Laya.Texture2DArray(width, height, capacity, format, false, false, sRGB);
        ...
        const virtualTex = new Laya.Texture(group.array);
    ...
    private static _getTexId(source: Laya.Texture | Laya.BaseTexture | Laya.Texture2D): number | null {
        if (!source)
            return null;
        // Texture → Texture2D
        if (source instanceof Laya.Texture) {
            const bmp = source.bitmap as Laya.Texture2D;
            return bmp._id
        }
