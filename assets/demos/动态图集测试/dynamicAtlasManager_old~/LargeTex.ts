...
export class LargeTex extends Laya.RenderTexture {
    cmdBuffer: Laya.CommandBuffer2D;
    private _limitMipmap: number = -1; //是否限制mipmap层数
    private _willDestroyTex: Laya.Texture2D[] = []; //待删除小贴图队列
    private _shader: Laya.Shader3D; //合图的着色器
    /**
     ...
     * @zh 命令
     */
    commands: Set<Laya.Blit2DCMD> = new Set();
    /**
     * @en Wait merge ids
     * @zh 等待合并的纹理ID
     */
    ...
    constructor(width: number, height: number, format: Laya.RenderTargetFormat = Laya.RenderTargetFormat.R8G8B8A8,
        depthStencilFormat: Laya.RenderTargetFormat = null, mipmap: boolean = false, limitMipmap: number = -1, sRGB: boolean = true, gammaCorrection: number = 1.0) {
        ...
        this._shader = Laya.Shader3D.find("TexMerge");

        // WebGPU：将渲染目标直接指向 Texture2DArray 的单层
        if (Config.useTextureArray) {
            const alloc = TextureArrayRegistry2D.allocateLayerAsTexture(width, height, Laya.TextureFormat.R8G8B8A8, 64, sRGB);
            const tc: any = Laya.LayaGL.textureContext;
            if (alloc && tc) {
                // 用数组层重建 RT
                this._disposeResource();
                // 内部纹理对象
                const internalArrayTex = (alloc.array as any)._texture;
                (this as any)._renderTarget = tc.createRenderTargetFromArrayLayer(internalArrayTex, alloc.layer, format,
        ...
        //
        (this as any)._texture.gammaCorrection = gammaCorrection;
    }
     ...
     * @param smallTex 小贴图
     */
    addTexture(x: number, y: number, w: number, h: number, expand: number, smallTex: Laya.Texture2D, needRemove: boolean) {
        let cmd = null;
        if (expand > 0)
     ...
     * @param format 贴图格式
     */
    addColor(x: number, y: number, w: number, h: number, color: Laya.Vector4, format: number) {
        let cf: Uint8Array;
        let smallTex: Laya.Texture2D;
        if (format == Laya.TextureFormat.R8G8B8) {
            const r = color.x * 255;
            const g = color.y * 255;
            const b = color.z * 255;
        ...
        else if (format == Laya.TextureFormat.R8G8B8A8) {
            const r = color.x * 255;
            const g = color.y * 255;
            const b = color.z * 255;
            const a = color.w * 255;
     ...
     * @param format 贴图格式
     */
    fillColor(color: Laya.Vector4, format: number) {
     ...
     * @returns 贴图对象
     */
    private _createSmallTex(w: number, h: number, pixelArray: Uint8Array): Laya.Texture2D {
        const smallTex = new Laya.Texture2D(w, h, this.format, false, false, false);
        smallTex.setPixelsData(pixelArray, false, false);
        smallTex.filterMode = Laya.FilterMode.Point;
        return smallTex;
    }
     ...
     * @param smallTex 小贴图
     */
    private _drawTex(x: number, y: number, w: number, h: number, expand: number, smallTex: Laya.Texture2D) {
        const width = this.width; //大贴图宽度
        const height = this.height; //大贴图高度
        const offsetScale = new Laya.Vector4(); //偏移和放缩系数
        ...
        //采用实时渲染方式将小贴图绘制到大贴图上
        let cmd = Laya.Blit2DCMD.create(smallTex, <any>this, offsetScale, this._shader, sd);
