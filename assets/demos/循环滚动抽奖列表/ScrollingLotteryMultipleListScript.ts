...
export type FixedSubLenCfg = {
    ...
    /** 子列表数据源填充选项（数组时，表示分别配置每个子列表的填充选项） {@linkcode Utils.repeatFillWithQuality} */
    subFillOptions?: {
        /** 品质 key */
        qualityKey: string,
        /** 最大连续相同品质次数，默认 2 */
...
//     // 子列表数据源填充选项（可选，数组时，表示分别配置每个子列表的填充选项）
//     subFillOptions: {
//         // 品质 key
//         qualityKey: xx,
//         // 最大连续相同品质次数（可选），默认 2
...
export class ScrollingLotteryMultipleListScript extends Laya.Script {
    ...
    private _flags: Flag;
    /** 当前使用的滚动条 */
    private _targetScrollBar: Laya.ScrollBar;
    /** 开奖结果最大索引数组 */
    ...
    public init(fixedSubLenCfg: FixedSubLenCfg = null): ScrollingLotteryMultipleListScript {
        ...
        for (let i = 0, c = this.owner.content.children.length; i < c; i++) {
            ...
            if (fixedSubLenCfg) {
                fixedLenCfg = {
                    ...
                    fillOptions: Array.isArray(fixedSubLenCfg.subFillOptions) ? fixedSubLenCfg.subFillOptions[subListIdx] : fixedSubLenCfg.subFillOptions
                };
            }
            // 子列表抽奖，初始化
            subLottery.init(fixedLenCfg);
            ...
            subLottery.onScrollStartHandler = new Laya.Handler(this, () => {
                ...
                this._tempParams.push(subLottery, subListIdx);
                this.owner.event(ScrollingLotteryMultipleListScript.EVENT_SCROLL_START, this._tempParams);
                this.onScrollStartHandler?.runWith(this._tempParams);
            });
            ...
            subLottery.onScrollingHandler = new Laya.Handler(this, (curFocusIdx: number) => {
                // 滚动中事件，聚焦索引改变
                this._tempParams.length = 0;
                this._tempParams.push(subLottery, subListIdx, curFocusIdx);
                this.owner.event(ScrollingLotteryMultipleListScript.EVENT_SCROLLING, this._tempParams);
                this.onScrollingHandler?.runWith(this._tempParams);
            });
            ...
            subLottery.onScrollProgressHandler = new Laya.Handler(this, (progress: number) => {
                // 滚动进度事件
                this._tempParams.length = 0;
                this._tempParams.push(subLottery, subListIdx, progress);
                this.owner.event(ScrollingLotteryMultipleListScript.EVENT_SCROLL_PROGRESS, this._tempParams);
                this.onScrollProgressHandler?.runWith(this._tempParams);
                // 父列表滚动到的子列表索引
            ...
            subLottery.onScrollCompleteHandler = new Laya.Handler(this, (curFocusIdx: number) => {
                ...
                // 滚动完成事件
                this._tempParams.length = 0;
                this._tempParams.push(subLottery, subListIdx, subCell);
                this.owner.event(ScrollingLotteryMultipleListScript.EVENT_SCROLL_COMPLETE, this._tempParams);
                this.onScrollCompleteHandler?.runWith(this._tempParams);
            });
        ...
        if (this.owner.parent && this.owner.parent instanceof Laya.Panel) { // 父级是Panel时
            ...
            if (panel.scrollType === Laya.ScrollType.Vertical) { // 垂直滚动Panel
                this._targetScrollBar = panel.vScrollBar;
            } else if (panel.scrollType === Laya.ScrollType.Horizontal) { // 水平滚动Panel
                this._targetScrollBar = panel.hScrollBar;
            }
        } else { // 父级非Panel时
            if (this.owner.scrollType === Laya.ScrollType.Vertical) { // 垂直滚动列表
                this._targetScrollBar = this.owner.scrollBar;
        ...
        Laya.timer.callLater(this, () => {
            Laya.timer.callLater(this, () => {
                ...
                if (this._targetScrollBar) {
                    this._targetScrollBar.changeHandler = new Laya.Handler(this, this.optimizeVisible);
                }
            });
        });
